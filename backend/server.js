...
// API to get users with highest message counts
app.get("/api/message-stats", authenticateToken, async (req, res) => {
  try {
    ...
    // Get user details for each sender ID
    const userPromises = messageCounts.map(async (item) => {
      return new Promise((resolve, reject) => {
        db.query('SELECT id, name, email FROM users WHERE id = ?', [item._id], (err, results) => {
          if (err) return reject(err);
          if (results.length === 0) return resolve({ id: item._id, name: "Unknown User", count: item.count });
          resolve({ 
            id: results[0].id, 
            name: results[0].name, 
    ...
    // Get conversation counts (chats initiated)
    const { Conversation } = require("./models/MessageModels");
    
    // This gets users who have participated in the most conversations
    ...
    // Get user details for each participant ID
    const chatUserPromises = conversationCounts.map(async (item) => {
      return new Promise((resolve, reject) => {
        db.query('SELECT id, name, email FROM users WHERE id = ?', [item._id], (err, results) => {
          if (err) return reject(err);
          if (results.length === 0) return resolve({ id: item._id, name: "Unknown User", count: item.count });
    ...
    const topChatUsers = await Promise.all(chatUserPromises);
    
    res.json({ 
      topMessageSenders: topUsers,
      topChatParticipants: topChatUsers
    });
    
  } catch (error) {
    console.error("Error fetching message statistics:", error);
    res.status(500).json({ message: "Failed to retrieve message statistics" });
  }
});
// API Routes
